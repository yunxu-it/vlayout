    ...
    if (requireLayoutView()) {
      ...
      if (!mLayoutRegion.isEmpty()) {
        ...
        if (helper.getOrientation() == VirtualLayoutManager.VERTICAL ? mLayoutRegion.intersects(0, -contentHeight / 4, contentWidth,
          ...
          if (helper.getOrientation() == VirtualLayoutManager.VERTICAL) {
            mLayoutRegion.left = helper.getPaddingLeft() + getFamilyMarginLeft() + getAncestorPaddingLeft();
            mLayoutRegion.right = helper.getContentWidth() - helper.getPaddingRight() - getFamilyMarginRight() - getAncestorPaddingRight();
          } else {
            mLayoutRegion.top = helper.getPaddingTop() + getFamilyMarginTop() + getAncestorPaddingTop();
            mLayoutRegion.bottom = helper.getContentWidth() - helper.getPaddingBottom() - getFamilyMarginBottom() - getAncestorPaddingBottom();
          }
    ...
    if (!rangeStyle.isChildrenEmpty()) {
      for (Map.Entry<Range<Integer>, T> entry : rangeStyle.mChildren.entrySet()) {
        ...
        if (childRangeStyle.mLayoutView != null) {
          rangeStyle.mLayoutRegion.union(childRangeStyle.mLayoutView.getLeft(), childRangeStyle.mLayoutView.getTop(), childRangeStyle.mLayoutView.getRight(),
            childRangeStyle.mLayoutView.getBottom());
        }
      }
    }
  }
